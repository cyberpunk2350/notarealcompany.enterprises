...
Gemini-generated 50-advisory sample set (BR-003 bio build phase) reviewed against established Brew 3000 canon. Approximately half the source entries disqualified on register or lore grounds: `SESSION SUMMARY` label (ARIA-exclusive, D-05); individual biometric sensing claims (heart rate, pupil dilation, cortisol — not established fleet capability); deliberate behavioral manipulation (temperature and acoustic interventions on occupants — the fleet responds to conditions, it does not engineer them); first-person interiority narration; fleet routing content to ARIA (wrong direction); visible punchline delivery (fleet does not land jokes); inter-unit editorial consensus (content undefined by design). Remaining entries substantially rewritten for register compliance. Canonical forms from MIN-2026-001 incorporated as reference standard. New inter-unit transmission format entries added (operational, not editorial). Curated to ~25 illustrative entries across seven category types. Published as Appendix H (`narc-primer-appendix-brew3k-advisory-samples.md`); index updated. BR-003v4, 2026-04-20.
